...
VTLA_CONFIG="${VTLA_CONFIG:-$PROJECT_ROOT/configs/vla/tacthru_umi/tacthru_umi_vtla_rgb_marker.yaml}"
...
if [[ ! -f "$VTLA_CONFIG" ]]; then
  echo "VTLA config not found: $VTLA_CONFIG" >&2
  exit 1
fi

export PYTHONPATH="$PROJECT_ROOT:$TACTHRU_REPO:${PYTHONPATH:-}"
cd "$PROJECT_ROOT"
exec "$REALMAN_PYTHON" tools/view_vtla_tactile_gate.py \
  --tacthru-repo "$TACTHRU_REPO" \
  --sensor-config "$TACTHRU_SENSOR_CONFIG" \
  --vtla-config "$VTLA_CONFIG" \
  "$@"
