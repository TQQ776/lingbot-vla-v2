...
def convert_fisheye_intrinsics_resolution(opencv_intr_dict: Dict[str, np.ndarray], target_resolution: Tuple[int, int]) -> Dict[str, np.ndarray]:
    """
...
def _detect_aruco_markers(img: np.ndarray, aruco_dict: cv2.aruco.Dictionary, param):
    if hasattr(cv2.aruco, "ArucoDetector"):
        detector = cv2.aruco.ArucoDetector(aruco_dict, param)
        return detector.detectMarkers(img)
    return cv2.aruco.detectMarkers(image=img, dictionary=aruco_dict, parameters=param)


def _single_marker_object_points(marker_size_m: float) -> np.ndarray:
...
def _estimate_single_marker_pose(corners: np.ndarray, marker_size_m: float, K: np.ndarray):
    if hasattr(cv2.aruco, "estimatePoseSingleMarkers"):
        try:
            rvec, tvec, marker_points = cv2.aruco.estimatePoseSingleMarkers(corners, marker_size_m, K, np.zeros((1, 5)))
            return rvec.squeeze(), tvec.squeeze(), marker_points
        except cv2.error:
            return None

    object_points = _single_marker_object_points(marker_size_m)
    image_points = np.asarray(corners, dtype=np.float32).reshape(-1, 2)
    if image_points.shape != (4, 2):
        return None
    dist_coeffs = np.zeros((4, 1), dtype=np.float64)
    solvepnp_flags = [getattr(cv2, "SOLVEPNP_IPPE_SQUARE", None), cv2.SOLVEPNP_ITERATIVE]
    for flag in solvepnp_flags:
        ...
        try:
            ...
            continue
        if ok:
            return rvec.squeeze(), tvec.squeeze(), object_points
    return None
...
def detect_localize_aruco_tags(
    ...
    param = _create_aruco_detector_parameters()
    if refine_subpix:
        param.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_SUBPIX
    corners, ids, rejectedImgPoints = _detect_aruco_markers(img=img, aruco_dict=aruco_dict, param=param)
    if len(corners) == 0:
        return dict()
    ...
    for this_id, this_corners in zip(ids, corners):
        ...
        pose_result = _estimate_single_marker_pose(undistorted, marker_size_m, K)
        if pose_result is None:
            continue
        rvec, tvec, markerPoints = pose_result
...
def get_charuco_board(
    ...
    board = cv2.aruco.CharucoBoard(size=grid_size, squareLength=square_length_mm / 1000, markerLength=tag_length_mm / 1000, dictionary=aruco_dict)
    return board
...
def inpaint_tag(img, corners, tag_scale=1.4, n_samples=16):
    ...
    sample_points = si.interp1d([0, 1, 2, 3, 4], list(scaled_corners) + [scaled_corners[0]], axis=0)(np.linspace(0, 4, n_samples)).astype(np.int32)
    sample_colors = img[np.clip(sample_points[:, 1], 0, img.shape[0] - 1), np.clip(sample_points[:, 0], 0, img.shape[1] - 1)]
